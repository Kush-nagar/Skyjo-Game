...
public class PlayerLayout extends JPanel implements MouseListener, MouseMotionListener {
    ...
    private void revealRemainingFaceDownCards() {
        ...
        for (Card card : playerCards) {
            if (!card.isFaceUp()) {
                card.setFaceUp(true); // Reveal any remaining face-down cards
            }
        }
        // Update the current player's score after revealing all cards
        playerScores.get(currentPlayer).updateScore(playerCards);
        repaint();
    }
    ...
    private void resetGame() {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                new HomeScreen(); // Launch the home screen again
            }
        });
    }
